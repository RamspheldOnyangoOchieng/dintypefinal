...
const ATTRIBUTES = {
  eyeColor: ['hazel', 'grey', 'brown', 'blue', 'green'],
  hairColor: ['silver', 'blue', 'black', 'brown', 'blonde', 'red'],
  hairStyle: ['bangs', 'short', 'long', 'straight', 'curly', 'bun', 'ponytail'],
...
async function generateWithNovita(prompt, category, value, style) {
  ...
  try {
    const res = await fetch('https://api.novita.ai/v3/async/txt2img', {
      ...
      body: JSON.stringify({
        ...
        request: {
          ...
          prompt: prompt,
          negative_prompt: 'man, male, boy, multiple people, low quality, distorted, bad anatomy, nudity, nsfw',
          width: 512,
          height: 768,
          image_num: 1,
          batch_size: 1,
    ...
    console.error(`Error in generateWithNovita: ${err.message}`);
    throw err;
  }
...
async function main() {
  console.log('Starting full attribute generation for the NEW database...');
