...
export function CharacterProvider({ children }: { children: ReactNode }) {
  ...
  async function fetchCharacters() {
    try {
      ...
      // Normalize image fields: prefer `image`, fall back to `imageUrl` (from image_url),
      // and resolve Supabase storage paths to public URLs when needed.
      const normalizedData = await Promise.all(
        (formattedData || []).map(async (c: any) => {
          try {
            ...
            // Otherwise treat it as a storage path and try to get a public URL
            // Normalize common cases: either 'characters/{file}' or just '{file}'
            ...
            try {
              ...
              console.warn('Failed to resolve public URL for', path, e)
            }
