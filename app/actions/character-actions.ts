...
export async function getCharacters() {
  try {
    ...
    // Normalize image fields and ensure valid URLs
    const normalizedData = (data || []).map(char => {
      // Prefer 'image' field, fallback to 'image_url'
      const imageUrl = char.image || char.image_url || ''
      
      // Only include characters with valid Supabase storage URLs
      if (!imageUrl || !imageUrl.includes('/storage/v1/object/public/images/')) {
        return null
      }
      
      // Ensure both fields are set to the same value for consistency
      return {
        ...char,
        image: imageUrl,
        image_url: imageUrl
